...
variable "internal_subnets" {
  ...
  default     = ["10.240.0.0/19","10.240.64.0/19","10.240.128.0/19"]
}
...
variable "external_subnets" {
  ...
  default     = ["10.240.32.0/20","10.240.96.0/20","10.240.160.0/20"]
}

variable "availability_zones" {
  type        = "list"
  description = "a comma-separated list of availability zones, defaults to all AZ of the region, if set to something other than the defaults, both internal_subnets and external_subnets have to be defined as well"
  /* Replace this with a reference to data source */
  /* The vpc resource call can be updated too at that point. */
  /* default     = [modules.defaults.zones] */
  default     = []
}
...
module "vpc" {
  ...
  availability_zones = "${split(",", coalesce(join(",", sort(var.availability_zones)), join(",", sort(module.defaults.zones))))}"
...
module "dhcp" {
  ...
  vpc_id  = "${module.vpc.id}"
  servers = "${coalesce(var.domain_name_servers, module.consul.domain_name_servers, module.defaults.domain_name_servers)}"
}
